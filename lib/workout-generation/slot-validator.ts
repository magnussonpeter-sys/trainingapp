...
function getRequiredCoverageSummary(params: {
  ...
  if (params.focus === "upper_body") {
    ...
    if (!(roles.has("main_hinge") || roles.has("recovery_light"))) {
      reasons.push("missing_hinge_or_glute");
    }
  } else if (params.focus === "recovery_strength") {
    if (!roles.has("main_pull")) reasons.push("missing_light_pull");
    if (!(roles.has("recovery_light") || roles.has("main_hinge"))) {
      reasons.push("missing_light_press_or_glute");
    }
    if (!(roles.has("core") || roles.has("rehab_control"))) {
      reasons.push("missing_light_core");
    }
  } else {
    if (!(roles.has("main_squat") || roles.has("unilateral_lower"))) {
    ...
    .filter((selection) =>
      ...
      !selection.candidates.some(
        (candidate) =>
          ...
          candidate.slotRole === selection.role,
      ),
    )

...
export async function insertWorkoutLog(input: CreateWorkoutLogInput) {
  ...
  try {
    ...
    // Sparar själva passet först och får tillbaka dess interna databas-id.
    ...
    // Sparar varje övning som hör till passet.
    for (let exerciseIndex = 0; exerciseIndex < input.exercises.length; exerciseIndex++) {
      ...
      // Sparar alla set för den aktuella övningen.
    ...
    // Sparar event om sådana skickats med.
...
export async function getWorkoutLogsByUser(userId: string, limit = 20) {
  ...
  // Returnerar i camelCase så att frontend kan använda datan direkt.
  return result.rows;
}

export async function getLatestExercisePerformance(userId: string, exerciseId: string) {
  const result = await pool.query(
    `
    select
      wl.completed_at as "completedAt",
    ...
    join workout_log_exercises wle
      on wle.workout_log_id = wl.id
    join workout_log_sets wls
      on wls.workout_log_exercise_id = wle.id
    where wl.user_id = $1
...
export async function deleteWorkoutLogById(userId: string, workoutLogId: string) {
  ...
  try {
    ...
    // Radera själva passet, men bara om det tillhör rätt användare.
...
return {
  // rowCount kan vara null enligt typen från pg, därför säkrar vi med ?? 0.
...
export async function deleteAllWorkoutLogsByUser(userId: string) {
  ...
  try {
    ...
    // Hämta alla övningar som hör till användarens pass.
    const exerciseIdsResult = await client.query<{ id: string }>(
      `
      select wle.id
      from workout_log_exercises wle
    ...
    // Radera alla events för användarens pass.
    await client.query(
      `
      delete from workout_log_events
